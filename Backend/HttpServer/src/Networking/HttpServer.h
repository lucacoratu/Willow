...
namespace Willow {
    ...
    private:
        ...
        std::map<std::pair<HttpMethod,std::string>, std::function<void(HttpRequest&, HttpResponse&)>> m_Handlers;

        //Handlers for different commands from stdin
        std::map<std::string, std::function<void()>> m_CommandHandlers;
    protected:
        ...
        //Handle for command received
        void OnCommandReceived(std::string command);
    public:
        ...
        //Add handler for a specific command from stdin
